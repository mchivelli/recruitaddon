...
public class MarchManager {
    ...
    private static List<Entity> findNearbyRecruits(Player player, ServerLevel level) {
        ...
        List<AbstractRecruitEntity> selectedRecruits = com.yourname.myforgemod.integration.RecruitsIntegration.getSelectedRecruits(player);
        if (!selectedRecruits.isEmpty()) {
            return new ArrayList<>(selectedRecruits);
        }
        List<AbstractRecruitEntity> nearbyRecruits = com.yourname.myforgemod.integration.RecruitsIntegration.findNearbyRecruits(player, 500);
    ...
    private static void setRecruitMarchTarget(Entity recruit, BlockPos target) {
        ...
        com.yourname.myforgemod.integration.RecruitsIntegration.sendMarchCommand(recruit, target);
    }
    
    private static void stopRecruit(Entity recruit) {
        // Send hold position command (march to current position)
        com.yourname.myforgemod.integration.RecruitsIntegration.sendMarchCommand(recruit, recruit.blockPosition());
    }
